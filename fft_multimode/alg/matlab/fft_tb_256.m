...
fd_re = fopen("x_re256.txt",'wb');
fd_im = fopen("x_im256.txt",'wb');
for i =1:N
...
fd_re = fopen("x_fft256_re.txt",'wb');
fd_im = fopen("x_fft256_im.txt",'wb');
for i =1:N
...
%%
%fft与ifft结果比较
f1=fopen('F:\A_my_project\FPGA\Digital_IC_class\fft_multimode\alg\data\y_re_fft256.txt','r');
[y_re_fft,num1]=fscanf(f1,'%04x',[1 inf]);
f2=fopen('F:\A_my_project\FPGA\Digital_IC_class\fft_multimode\alg\data\y_im_fft256.txt','r');
...
f1=fopen('F:\A_my_project\FPGA\Digital_IC_class\fft_multimode\alg\data\y_re_ifft256.txt','r');
[y_re_ifft,num1]=fscanf(f1,'%04x',[1 inf]);
f2=fopen('F:\A_my_project\FPGA\Digital_IC_class\fft_multimode\alg\data\y_im_ifft256.txt','r');
...
figure(5)
subplot(2,1,1);plot(k,X_std,'LineWidth',1.5);title('256点fft标准结果')
grid on
subplot(2,1,2);plot(k,X_sim,'LineWidth',1.5);title('256点fft仿真结果')
grid on
% title('256点fft结果比较');
% legend({'标准结果','仿真结果'});
...
figure(6)
subplot(2,1,1);plot(k,X,'LineWidth',1.5);title('256点ifft标准结果')
grid on
subplot(2,1,2);plot(k,y_re_ifft_singed,'LineWidth',1.5);title('256点ifft仿真结果')
grid on
% title('256点ifft结果比较');
% legend({'标准结果','仿真结果'});